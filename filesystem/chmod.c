...
fchmod:- Set permission to User ,Group, and Other Based on the modes


Syntax:-

    #include <sys/stat.h>
    int fchmod(file discriptor, mode_t mode);
...
int main()
{
   ...
   if(fd >= 0 )
   {
    printf("file opened successfully\n");
      ...
      int status = chmod(path,S_IXUSR |S_IRUSR |S_IWUSR |S_IXGRP |S_IRGRP |S_IWGRP |S_IXOTH |S_IROTH |S_IWOTH );
